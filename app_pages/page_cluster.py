...
def page_cluster_body():
    try:
        ...
        if 'Clusters' not in df_total_vs_clusters.columns:
            df_total_vs_clusters['Clusters'] = cluster_pipe['model'].labels_

        st.write("### ML Pipeline: Cluster Analysis")
        st.info(
            f"* I refitted the cluster pipeline using fewer variables, and it delivered equivalent "
...
def cluster_distribution_per_variable(df, target):
    try:
        ...
        st.error(f"An error occurred in cluster distribution: {e}")
